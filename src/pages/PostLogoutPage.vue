    ...
    <q-card class="post-logout-card">
        ...
        <div class="info-section q-mb-lg">
          <div class="row items-start q-mb-md">
            ...
            <div class="text-body2 text-grey-7">
              Your games, sessions, and other gaming data remain stored locally on this device. Logging out does not delete your personal information.
            </div>
          </div>
      ...
      <q-card-section class="post-logout-footer">
        <div class="row items-center justify-center q-gutter-md">
          <q-btn flat color="grey-6" size="sm" @click="goToAbout"> About Playloop </q-btn>
        </div>
      </q-card-section>
    </q-card>
  </q-page>
...
async function clearLocalData() {
  const confirmed = await dialogService.confirm(
    ...
    "Are you sure you want to remove all local data from this device? This action cannot be undone. All your games, sessions, and settings will be permanently deleted from this device. However, no data will be deleted from the remote server."
  );
  if (!confirmed) return;
  try {
...
async function showVersionInfo() {
  const title = `Playloop v${APP_VERSION}`;
